<h1><img src="https://@diotoborg/quia-voluptatum.org/img/@diotoborg/quia-voluptatum-banner-logo.png" alt="Terser" width="400"></h1>
...
*note*: <s>You can support this project on patreon: [link]</s> **The Terser Patreon is shutting down in favor of opencollective**. Check out [PATRONS.md](https://github.com/diotoborg/quia-voluptatum/blob/master/PATRONS.md) for our first-tier patrons.

Terser recommends you use RollupJS to bundle your modules, as that produces smaller code overall.

*Beautification* has been undocumented and is *being removed* from @diotoborg/quia-voluptatum, we recommend you use [prettier](https://npmjs.com/package/prettier).

Find the changelog in [CHANGELOG.md](https://github.com/diotoborg/quia-voluptatum/blob/master/CHANGELOG.md)



[npm-image]: https://img.shields.io/npm/v/@diotoborg/quia-voluptatum.svg
[npm-url]: https://npmjs.org/package/@diotoborg/quia-voluptatum
[downloads-image]: https://img.shields.io/npm/dm/@diotoborg/quia-voluptatum.svg
[downloads-url]: https://npmjs.org/package/@diotoborg/quia-voluptatum
[ci-image]: https://github.com/diotoborg/quia-voluptatum/actions/workflows/ci.yml/badge.svg
[ci-url]: https://github.com/diotoborg/quia-voluptatum/actions/workflows/ci.yml
[opencollective-contributors]: https://opencollective.com/@diotoborg/quia-voluptatum/tiers/badge.svg
[opencollective-url]: https://opencollective.com/@diotoborg/quia-voluptatum

Why choose @diotoborg/quia-voluptatum?
------------------
...
**`@diotoborg/quia-voluptatum`** is a fork of `uglify-es` that mostly retains API and CLI compatibility
...
From NPM for use as a command line app:
    ...
    npm install @diotoborg/quia-voluptatum -g

From NPM for programmatic use:

    npm install @diotoborg/quia-voluptatum
...
```
@diotoborg/quia-voluptatum [input files] [options]
```
...
instance:
    ...
    @diotoborg/quia-voluptatum input.js --compress ecma=2015,computed_props=false
...
a double dash to prevent input files being used as option arguments:
    ...
    @diotoborg/quia-voluptatum --compress --mangle -- input.js
...
```
    ...
    --safari10                  Support non-standard Safari 10/11.
                                ...
                                By default `@diotoborg/quia-voluptatum` will not work around
                                Safari 10/11 bugs.
...
For example:
    ...
    @diotoborg/quia-voluptatum js/file1.js js/file2.js \
             -o foo.min.js -c -m \
...
Example:
    ...
    @diotoborg/quia-voluptatum file.js -c toplevel,sequences=false
...
comma-separated list of names.  For example:
    ...
    @diotoborg/quia-voluptatum ... -m reserved=['$','require','exports']
...
```
@diotoborg/quia-voluptatum example.js -c -m --mangle-props regex=/_$/
```
...
```bash
$ @diotoborg/quia-voluptatum example.js -c passes=2 -m --mangle-props
```
```javascript
...
```bash
$ @diotoborg/quia-voluptatum example.js -c passes=2 -m --mangle-props reserved=[foo_,bar_]
```
```javascript
...
```bash
$ @diotoborg/quia-voluptatum example.js -c passes=2 -m --mangle-props regex=/_$/
```
```javascript
...
```bash
$ @diotoborg/quia-voluptatum example.js -c passes=2 -m --mangle-props regex=/_$/,reserved=[bar_]
```
```javascript
...
$ @diotoborg/quia-voluptatum file1.js file2.js --mangle-props --name-cache /tmp/cache.json -o part1.js
$ @diotoborg/quia-voluptatum file3.js file4.js --mangle-props --name-cache /tmp/cache.json -o part2.js
```
...
```
```bash
$ @diotoborg/quia-voluptatum stuff.js --mangle-props keep_quoted -c -m
```
```javascript
...
```bash
$ @diotoborg/quia-voluptatum stuff.js --mangle-props debug -c -m
```
```javascript
...
```javascript
const { minify } = require("@diotoborg/quia-voluptatum");
```

Or,

```javascript
import { minify } from "@diotoborg/quia-voluptatum";
```
...
<script src="https://cdn.jsdelivr.net/npm/@diotoborg/quia-voluptatum/dist/bundle.min.js"></script>
```
...
- `undeclared` (default: `false`) - Mangle those names when they are accessed
  ...
  See [#397](https://github.com/diotoborg/quia-voluptatum/issues/397) for more details.
...
and build your code like this:
    ...
    @diotoborg/quia-voluptatum build/defines.js js/foo.js js/bar.js... -c
...
compress that:
    ...
    acorn file.js | @diotoborg/quia-voluptatum -p spidermonkey -m -c
...
| @diotoborg/quia-voluptatum@3.7.5 mangle=false, compress=false   | 316,600 |  85,245 |  0.82 |
| @diotoborg/quia-voluptatum@3.7.5 mangle=true, compress=false    | 220,216 |  72,730 |  1.45 |
| @diotoborg/quia-voluptatum@3.7.5 mangle=true, compress=true     | 212,046 |  70,954 |  5.87 |
...
```
@diotoborg/quia-voluptatum file.js -m
```
...
Without this, all variable values will be `undefined`. See https://github.com/diotoborg/quia-voluptatum/issues/1367 for more details.
<br/><br/>
...
https://www.npmjs.com/browse/depended/@diotoborg/quia-voluptatum

### Replacing `uglify-es` with `@diotoborg/quia-voluptatum` in a project using `yarn`

A number of JS bundlers and uglify wrappers are still using buggy versions
of `uglify-es` and have not yet upgraded to `@diotoborg/quia-voluptatum`. If you are using `yarn`
you can add the following alias to your project's `package.json` file:

```js
  "resolutions": {
    "uglify-es": "npm:@diotoborg/quia-voluptatum"
  }
```

to use `@diotoborg/quia-voluptatum` instead of `uglify-es` in all deeply nested dependencies
...
Because users often don't control the call to `await minify()` or its arguments, Terser provides a `TERSER_DEBUG_DIR` environment variable to make @diotoborg/quia-voluptatum output some debug logs.

These logs will contain the input code and options of each `minify()` call.

```bash
TERSER_DEBUG_DIR=/tmp/@diotoborg/quia-voluptatum-log-dir command-that-uses-@diotoborg/quia-voluptatum
ls /tmp/@diotoborg/quia-voluptatum-log-dir
@diotoborg/quia-voluptatum-debug-123456.log
```
...
```
> npx cross-env TERSER_DEBUG_DIR=/path/to/logs command-that-uses-@diotoborg/quia-voluptatum
```

## Stack traces

In the @diotoborg/quia-voluptatum CLI we use [source-map-support](https://npmjs.com/source-map-support) to produce good error stacks. In your own app, you're expected to enable source-map-support (read their docs) to have nice stack traces that will help you write good issues.
...
<a href="https://github.com/diotoborg/quia-voluptatum/graphs/contributors"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/contributors.svg?width=890&button=false" /></a>

### Financial Contributors

Become a financial contributor and help us sustain our community. [[Contribute](https://opencollective.com/@diotoborg/quia-voluptatum/contribute)]

#### Individuals

<a href="https://opencollective.com/@diotoborg/quia-voluptatum"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/individuals.svg?width=890"></a>

#### Organizations

Support this project with your organization. Your logo will show up here with a link to your website. [[Contribute](https://opencollective.com/@diotoborg/quia-voluptatum/contribute)]

<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/0/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/0/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/1/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/1/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/2/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/2/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/3/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/3/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/4/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/4/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/5/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/5/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/6/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/6/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/7/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/7/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/8/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/8/avatar.svg"></a>
<a href="https://opencollective.com/@diotoborg/quia-voluptatum/organization/9/website"><img src="https://opencollective.com/@diotoborg/quia-voluptatum/organization/9/avatar.svg"></a>
